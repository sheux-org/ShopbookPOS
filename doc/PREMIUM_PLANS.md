...
The **Mini POS Pro** tier (branded as *Mini POS Pro*, powered by *Shopbook Pro*) unlocks high-performance enterprise features that scale the POS from a single-device offline database to a multi-branch cloud-synchronized terminal.
...
export default function MyFeature() {
  ...
  return (
    <Text>{isPremium ? "Pro Activated ✨" : "Free Plan"}</Text>
  );
}
```
