...
- [ ] Create **two** secret API keys (Project → API keys). Never ship either in an app.
  - [ ] **v1 secret key** → Supabase secret `RC_SECRET_API_KEY`. Used by the webhook's `GET /v1/subscribers/{id}` re-fetch.
  - [ ] **v2 secret key** with permission `customer_information:customers:read_write` → kept by ops for promotional grants (build plan §7). **v1 keys are rejected by v2 endpoints**, which is why this cannot be the same key.
...
- [ ] **Webhook** (Integrations → Webhooks): URL `https://<project-ref>.functions.supabase.co/revenuecat-webhook`; environment _Production + Sandbox_; all event types. Use _Send test event_ after the function is deployed.
  - [ ] _Authorization header value_ = a long random string → Supabase secret `RC_WEBHOOK_AUTH`.
  - [ ] Copy the integration's **signing secret** → Supabase secret `RC_WEBHOOK_SIGNING_SECRET`. The function prefers HMAC verification and falls back to the header only when this is unset.
