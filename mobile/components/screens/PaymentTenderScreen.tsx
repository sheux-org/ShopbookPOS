...
export const PaymentTenderScreen: React.FC = () => {
  ...
  const discountValue = params.discountValue
    ? parseFloat(params.discountValue as string)
    : undefined;
  ...
  const handleCompleteSale = () => {
    if (activeMethod === 'cash' && parsedTendered < totalAmount) {
      ...
      Alert.alert(
        'Insufficient Tender',
        `Amount tendered must be at least Rs. ${totalAmount.toLocaleString()}`
      );
      return;
    }
    ...
    if (activeMethod === 'card') {
      if (!selectedBank) {
        ...
        Alert.alert(
          'Bank Required',
          'Please select a Sri Lankan bank to complete the card transaction.'
        );
        return;
      }
      ...
      <View style={styles.selectorTabsRow}>
        ...
        >
          <Feather
            name="pocket"
            size={14}
            color={activeMethod === 'cash' ? TOKENS.primary : TOKENS.muted}
          />
          <Text
            style={[
        ...
        >
          <Feather
            name="credit-card"
            size={14}
            color={activeMethod === 'card' ? TOKENS.primary : TOKENS.muted}
          />
          <Text
            style={[
              styles.selectorTabText,
        ...
        <View style={styles.modalBg}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalInvoice}>
              Invoice {createdInvoiceNumber ? getInvoiceLabel(createdInvoiceNumber) : '##2041'}{' '}
              Approved
            </Text>
      ...
      >
        <ScrollView style={styles.bankListScrollView} keyboardShouldPersistTaps="handled">
          {SRI_LANKAN_BANKS.map((bank) => (
            ...
            >
              <Text
                style={[styles.bankItemText, selectedBank === bank && styles.bankItemTextActive]}
              >
                {bank}
              </Text>
              {selectedBank === bank && <Feather name="check" size={16} color={TOKENS.primary} />}
            </TouchableOpacity>
          ))}
        </ScrollView>
      </BottomSheet>
...
const styles = StyleSheet.create({
  ...
  simCard: {
    ...
    padding: 20,
    justifyContent: 'space-between',
    boxShadow: '0px 6px 10px 0px rgba(0, 0, 0, 0.35)',
  },
  simCardHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
  ...
  successIconWrapper: {
    ...
    justifyContent: 'center',
    marginBottom: 16,
    boxShadow: `0px 4px 6px 0px ${TOKENS.success}4D`,
  },
  modalTitle: {
    fontSize: 20,
    fontWeight: 'bold',
