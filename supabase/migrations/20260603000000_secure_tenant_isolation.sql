-- =========================================================================
-- SECURE TENANT ISOLATION MIGRATION (SHOPBOOK POS) - REVISED FOR ANON SYNC
-- 20260603000000_secure_tenant_isolation.sql
-- =========================================================================

-- 1. Alter deleted_records table to include business_id
ALTER TABLE public.deleted_records ADD COLUMN IF NOT EXISTS business_id text;

-- 2. Update record_deletion trigger function to log business_id
...
-- Drop legacy functions if they exist with old signatures to avoid parameter mismatch
DROP FUNCTION IF EXISTS pull_watermelondb_changes(bigint);
DROP FUNCTION IF EXISTS push_watermelondb_changes(json);

-- 3. Redefine pull_watermelondb_changes function to take client_business_id parameter
...
-- 4. Redefine push_watermelondb_changes function to take client_business_id parameter
...
DROP POLICY IF EXISTS "Allow business active_devices delete" ON public.active_devices;
